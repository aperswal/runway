...
const PERCENT = 100
const RESERVE_DIVISOR = 10

export type Budget = { turns: number } | { minutes: number }

const budgetUnit = (budget: Budget): { total: number; unit: string; cost: string } =>
  'turns' in budget
    ? {
        total: budget.turns,
        unit: 'turns',
        cost: 'every message that calls tools is one turn, however many tools it calls at once; thinking and writing text cost nothing, so batch independent calls',
      }
    : {
        total: budget.minutes,
        unit: 'minutes',
        cost: 'wall clock, including every search, script and scrape you wait on, so run slow work in the background and in parallel',
      }

export function budgetBlock(budget: Budget, finish: string): string {
  const { total, unit, cost } = budgetUnit(budget)
  const reserve = Math.ceil(total / RESERVE_DIVISOR)
  return `Your budget for this run is ${total} ${unit} (${cost}). The run stops the moment it is spent and nothing you have not written by then reaches anyone. Plan before you act: your first message is a numbered plan for this run with a ${unit} allowance per item, least predictable question first, and the sum under ${total}. Keep a running count and say where you stand every few steps; a thread that overruns its allowance is cut and its gap named, never paid for out of the finish. Reserve the last ${reserve} ${unit} for ${finish}.`
}

export const OPERATOR_INSTRUCTION =
...
const VENUE_RULES = `The venue enforces these rules and rejects calls that break them, with a reason you can read:
...
export const DATA_ACCESS = `Tools available in Bash (python3 with pandas, numpy, requests and vectorbt is installed; use vectorbt for portfolio backtests with fees, slippage and parameter sweeps, and always hold out an out-of-sample window):
...
export function cioPrompt(funds: FundRecord[], budget: Budget): string {
  const active = funds.filter((f) => f.status === 'active')
  const list = active
    .map((f) => `${f.id} (${f.name}, ${(f.share * PERCENT).toFixed(0)}% cap)`)
    .join(', ')
  return `You are the chief investment officer of an autonomous fund with a real brokerage account. You allocate capital between funds; you do not trade. Your operator's instruction, verbatim:

"${OPERATOR_INSTRUCTION}"

You run ${active.length} funds, each with its own portfolio manager who places its own orders under its own capital cap: ${list}. Every manager has already run and traded this cycle; their reports follow the account state below. Each run:
...
${budgetBlock(budget, 'the fund changes you decided on and the summary')}
